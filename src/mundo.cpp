...
void Mundo::moverPieza(Coordenadas origen, Coordenadas destino) {
    ...
    if(pieza){
		...
		if (esJaque(turnoBlanco ? BLANCO : NEGRO)) {
			Pieza* destinoPieza = obtenerPiezaEn(destino);
			if(!movimientoEvitaJaque(pieza, destino) && (!destinoPieza || destinoPieza->getColor() == pieza->getColor())) {
...
bool Mundo::movimientoEvitaJaque(Pieza* pieza, Coordenadas destino){
	if (!pieza) return false; // Verificar que pieza no sea nullpt
